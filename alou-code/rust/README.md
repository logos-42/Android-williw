# 🦞 Alou Code — Rust Implementation

A high-performance Rust rewrite of the Alou Code CLI agent harness. Built for speed, safety, and native tool execution.
...
| Config file hierarchy (`.alou.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
`alou acp` is a local discoverability surface for editor-first users: it reports the current ACP/Zed status without starting the runtime. As of April 16, 2026, alou-code does **not** ship an ACP/Zed daemon entrypoint yet, and `alou acp serve` is only a status alias until the real protocol surface lands.
...
Notable alou-first surfaces now available directly in slash form:
...
└── crates/
    ...
    ├── rusty-claude-cli/   # Main CLI binary (`alou`)
