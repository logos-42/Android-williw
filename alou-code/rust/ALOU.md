...
This file provides guidance to Alou Code (clawcode.dev) when working with code in this repository.
...
- Keep shared defaults in `.alou.json`; reserve `.alou/settings.local.json` for machine-local overrides.
- Do not overwrite existing `ALOU.md` content automatically; update it intentionally when repo workflows change.
